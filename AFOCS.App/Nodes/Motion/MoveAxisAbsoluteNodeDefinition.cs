...
/// <summary>
/// 轴绝对运动节点：选择轴，按给定位置做绝对运动。
/// 工位由入口节点传入（context["WorkPos"]），总线轴使用定长运动（posiMode=1 绝对模式），
/// 雅克贝斯轴使用绝对运动接口。
/// </summary>
[Export]
[Export(typeof(INodeDefinition))]
[PartCreationPolicy(CreationPolicy.NonShared)]
[NodeDefinition("App.MoveAxisAbsolute", "轴绝对运动", "运动")]
[CategoryOrder("基础", 0),
 CategoryOrder("配置", 1),
 CategoryOrder("输入", 2),
 CategoryOrder("输出", 3)]
[method: ImportingConstructor]
public class MoveAxisAbsoluteNodeDefinition(IBusAxisDevice busAxisDevice,
    ...
    : NodeDefinitionBase, IExecutableNode
{
    ...
    [DisplayName("位置")]
    [NodePort("Position", "位置", NodePortType.Double, true)]
    [Category("输入")]
    ...
    public async Task<Dictionary<string, object?>> ExecuteAsync(Dictionary<string, object?> context)
    {
        ...
        if (Axis.IsBusAxis())
        {
            ...
            var moveResult = await busAxisDevice.MovePmoveAsync(busId, Position, posiMode: 1);
        ...
        else if (Axis.IsAkribisAxis())
        {
            ...
            {
                var akResult = await motion.MoveAbsAsync(akAxis, (int)Position);
