...
namespace AFOCS.App.Nodes.Motion
{
    ...
    public class MoveToTeachingPointNodeDefinition : NodeDefinitionBase, IExecutableNode
    {
        ...
        public MoveToTeachingPointNodeDefinition(IConfigService configService)
        {

        }
        public async Task<Dictionary<string, object?>> ExecuteAsync(Dictionary<string, object?> context)
        {
            // 节点实例由模板浅克隆而来，服务引用不会随之复制，因此通过静态定位器获取
            ...
            var tasks = axisKeys
                ...
                .Select(axis => MoveSingleAxisAsync(axis, positions[axis], station, _busAxisDevice, akribisInstances))
                .ToArray();
            ...
            //if (errors.Count > 0)
            //{
            //    ResultMessage = $"运动失败: {string.Join("; ", errors)}";
            //    throw new InvalidOperationException(ResultMessage);
            //}

            //ResultMessage = $"已到达示教点 \"{point.Name}\"（{station.GetDescription()}）";
    ...
    /// <summary>
    /// 示教点数据源：为属性面板的"示教点"提供下拉列表。
    /// 每次展开下拉时实时从配置加载（清除缓存保证拿到最新示教点）。
    /// 下拉项显示 "名称（工位）"，选中的值是示教点 Guid。
    /// </summary>
    public class TeachingPointItemsSource : IItemsSource
    {
        ...
        {
            var configService = AppBootstrapper.GetInstance<IConfigService>();
            return Task.Run(() =>
            {
                configService.ClearCache<TeachingPointsConfig>();
                return configService.LoadAsync<TeachingPointsConfig>();
