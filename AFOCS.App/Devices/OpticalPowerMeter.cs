...
namespace AFOCS.App.Devices
{
    public class OpticalPowerMeter : IOpticalPowerMeter
    {
        ...
        public OpticalPowerMeter(WorkPos workPos)
        {
            WorkPos = workPos;
            _tcpClient = IoC.Get<ITcpClient>();
            _configService = IoC.Get<IConfigService>();
            _logger = IoC.Get<ILogger<OpticalPowerMeter>>();
        }
        ...
        {
            if (!IsConnected) return Result<bool>.Fail(ResultCode.Fail, "未连接设备");
            try
            {
        ...
        {
            if (!IsConnected) return Result<(OSType, int)>.Fail(ResultCode.Fail, "未连接设备");
            try
            {
        ...
        {
            if (!IsConnected) return Result<double>.Fail(ResultCode.Fail, "未连接设备");
            try
            {
